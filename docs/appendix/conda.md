...
ライセンスへの同意後に、conda のインストール先を聞かれます。特別な理由が無い限り変更せずに、`>>>` に続けて「Enter」キーを押します。
...
最後にインストールスクリプトを削除してインストールを終えます。ターミナルを再起動すると conda が利用できるようになります。
...
次に、インストール先のフォルダを設定します。基本、デフォルトのままにします。ただし、ユーザー名に空白などが含まれている場合に、一部のソフトウェアでは対応できないため、空白を含まないようなフォルダを指定してください。
...
```{image} ../_static/conda-windows-07.png
:alt: Miniconda Windows インストール完了
:width: 80%
:align: center
```

最後に環境変数を設定するところでは、チェックボックスをすべて外して「Finish」をクリックします。

```{image} ../_static/conda-windows-08.png
:alt: Miniconda Windows 環境変数設定
:width: 80%
:align: center
```
